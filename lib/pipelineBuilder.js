...
// Either String Field
const toField = (Model, Models, field) => {
    ...
    const populationModel = getPopulationModel(localFieldModel, Models);
...
const pathToFieldList = (Model, Models, path) => {
    ...
    const eitherFieldList = parts.reduce(
        (eFieldList, fieldName) =>
            eFieldList.chain(fieldList => {
                ...
                return toField(model, Models, fieldName).map(f => fieldList.concat(f));
            }),
        Either.of([])
    );
...
const createPopulationTrees = (Model, Models, population) =>
    traverse(Either.of, path => pathToFieldList(Model, Models, path), population).map(fieldLists =>
        fieldLists
            .map(fieldListToTree)
...
const populationPipeline = (Model, Models, populate) =>
    createPopulationTrees(Model, Models, populate)
...
// Remove fields
const removeFieldsPopulated = (pipeline) => {
    return pipeline.reduce((fieldsToRemove, item) => {
        if (item.$addFields) {
            const key = Object.keys(item.$addFields)[0];
            if (key.includes('temp')) {
                fieldsToRemove.push(...[{
                    $addFields: {
                        [item.$addFields[key].replace("$", "")] : "$" + key
                    }
                },{
                    $unset: key
                }])
...
const build = (Model, Models, options) => {
    const {
        find, // search object
        sort, // sort object
        skip = 0, // int
    ...
    return populationPipeline(Model, Models, populate).map(pipeline => [
        ...pipeline,
        ...(find ? [{ $match: parseQuery(find) }] : []),
        ...(sort ? [{ $sort: sort }] : []),
        ...(unsetPopulatedFields ? removeFieldsPopulated(pipeline.reverse()) : []),
